...
use agentii_core::green_contract::{GreenContract, GreenContractOutcome, GreenLevel};
use agentii_core::{
...
fn worker_provider_failure_flows_through_recovery_to_policy() {
    use agentii_core::recovery_recipes::{
        attempt_recovery, FailureScenario, RecoveryContext, RecoveryResult, RecoveryStep,
    };
    use agentii_core::worker_boot::{WorkerFailureKind, WorkerRegistry, WorkerStatus};
    ...
    assert!(
        ctx.events().iter().any(|e| {
            matches!(
                e,
                agentii_core::recovery_recipes::RecoveryEvent::RecoveryAttempted {
