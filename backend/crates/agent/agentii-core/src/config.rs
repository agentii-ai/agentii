...
pub const AGENTII_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".agentii.json"),
            |parent| parent.join(".agentii.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".agentii.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".agentii").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".agentii").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("AGENTII_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".agentii")))
        .unwrap_or_else(|| PathBuf::from(".agentii"))
}
...
fn read_optional_json_object(path: &Path) -> Result<Option<ParsedConfigFile>, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".agentii.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, AGENTII_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".agentii");
        fs::create_dir_all(cwd.join(".agentii")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".agentii.json"),
        ...
        assert_eq!(AGENTII_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".agentii");
        let project_settings = cwd.join(".agentii").join("settings.json");
        fs::create_dir_all(cwd.join(".agentii")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
