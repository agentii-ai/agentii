...
///
/// Tier mapping for CLI `--tier` flag and agentii permission model:
/// - `suggest` → [`PermissionMode::ReadOnly`]: read-only tools only, no file writes or bash mutations
/// - `auto-edit` → [`PermissionMode::WorkspaceWrite`]: file edits within workspace root allowed, destructive bash prompted
/// - `full-auto` → [`PermissionMode::DangerFullAccess`]: all tools permitted without prompting
