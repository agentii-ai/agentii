...
/// Read the `.agentii-base` file from the given directory and return the trimmed
/// commit hash, or `None` when the file is absent or empty.
pub fn read_claw_base_file(cwd: &Path) -> Option<String> {
    let path = cwd.join(".agentii-base");
...
/// fall back to reading `.agentii-base` from `cwd`.
...
mod tests {
    ...
    fn reads_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".agentii-base"), "abc1234def5678\n").expect("write .agentii-base");
    ...
    fn returns_none_for_empty_claw_base_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".agentii-base"), "  \n").expect("write empty .agentii-base");
    ...
    fn resolve_expected_base_prefers_flag_over_file() {
        ...
        let root = temp_dir();
        fs::create_dir_all(&root).expect("create dir");
        fs::write(root.join(".agentii-base"), "from_file\n").expect("write .agentii-base");
    ...
    fn matches_with_claw_base_file_in_real_repo() {
        ...
        init_repo(&root);
        let sha = head_sha(&root);
        fs::write(root.join(".agentii-base"), format!("{sha}\n")).expect("write .agentii-base");
        let source = resolve_expected_base(None, &root);
    ...
    fn diverged_with_claw_base_file_after_new_commit() {
        ...
        init_repo(&root);
        let old_sha = head_sha(&root);
        fs::write(root.join(".agentii-base"), format!("{old_sha}\n")).expect("write .agentii-base");
