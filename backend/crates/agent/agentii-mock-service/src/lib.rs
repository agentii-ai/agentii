...
use agentii_api::{InputContentBlock, MessageRequest, MessageResponse, OutputContentBlock, Usage};
...
impl Scenario {
    fn parse(value: &str) -> Option<Self> {
        match value.trim() {
            ...
            "agent_fork" => Some(Self::AgentFork),
...
async fn handle_connection(
    ...
    // count_tokens preflight — return a minimal valid response without capturing.
...
fn flatten_tool_result_content(content: &[agentii_api::ToolResultContentBlock]) -> String {
    content
        .iter()
        .map(|block| match block {
            agentii_api::ToolResultContentBlock::Text { text } => text.clone(),
            agentii_api::ToolResultContentBlock::Json { value } => value.to_string(),
        })
        .collect::<Vec<_>>()
        .join("\n")
}
...
fn build_stream_body(request: &MessageRequest, scenario: Scenario) -> String {
    match scenario {
        ...
        // Session resume: return a simple text response confirming the resumed session context.
        Scenario::SessionResume => {
            final_text_sse("session resume parity complete. prior messages visible.")
        }
        // Agent fork: mock returns a text response that includes a sub-agent result marker.
        Scenario::AgentFork => {
            final_text_sse("agent fork parity complete. sub-agent result: fork-result-parity")
        }
        // Error recovery: first call returns 500 (handled by retry logic in the runtime),
        // subsequent call returns success. The mock always returns success here because
        // the retry is exercised at the HTTP client level, not the mock level.
        Scenario::ErrorRecovery => {
            final_text_sse("error recovery parity complete. retried successfully.")
        }
    }
}
...
fn build_message_response(request: &MessageRequest, scenario: Scenario) -> MessageResponse {
    match scenario {
        ...
        Scenario::SessionResume => text_message_response(
            "msg_session_resume",
            "session resume parity complete. prior messages visible.",
        ),
        Scenario::AgentFork => text_message_response(
            "msg_agent_fork",
            "agent fork parity complete. sub-agent result: fork-result-parity",
        ),
        Scenario::ErrorRecovery => text_message_response(
            "msg_error_recovery",
            "error recovery parity complete. retried successfully.",
        ),
    }
}
...
fn request_id_for(scenario: Scenario) -> &'static str {
    match scenario {
        ...
        Scenario::SessionResume => "req_session_resume",
        Scenario::AgentFork => "req_agent_fork",
        Scenario::ErrorRecovery => "req_error_recovery",
    }
}
