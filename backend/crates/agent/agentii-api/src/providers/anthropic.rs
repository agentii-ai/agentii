...
use agentii_telemetry::{AnalyticsEvent, AnthropicRequestProfile, ClientIdentity, SessionTracer};
    ...
    F: FnOnce() -> Result<Option<OAuthConfig>, ApiError>,
{
    ...
    // Try saved OAuth credentials before giving up
    if let Some(config) = load_oauth_config()? {
        if let Ok(Some(core_token_set)) = agentii_core::load_oauth_credentials() {
            let token_set = OAuthTokenSet {
                access_token: core_token_set.access_token,
                refresh_token: core_token_set.refresh_token,
                expires_at: core_token_set.expires_at,
                scopes: core_token_set.scopes,
            };
...
mod tests {
    ...
    use agentii_core::{clear_oauth_credentials, save_oauth_credentials, OAuthConfig};
    ...
    fn resolve_saved_oauth_token_refreshes_expired_credentials() {
        ...
        let stored = agentii_core::load_oauth_credentials()
    ...
    #[test]
    fn resolve_startup_auth_source_uses_saved_oauth_when_no_env_vars() {
        let _guard = env_lock();
        let config_home = temp_config_home();
        std::env::set_var("AGENTII_CONFIG_HOME", &config_home);
        std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
        ...
        let auth = resolve_startup_auth_source(|| {
            Ok(Some(OAuthConfig {
                ...
                authorize_url: "http://localhost:0/authorize".to_string(),
                callback_port: None,
                manual_redirect_url: None,
                scopes: vec!["scope:a".to_string()],
            }))
        })
        .expect("saved non-expired oauth should resolve");
