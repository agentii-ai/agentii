...
pub enum ProviderKind {
    ...
    // T108: Ollama
    Ollama,
    // T109: Bedrock (Anthropic-compatible)
    Bedrock,
    // T110: Azure OpenAI
    AzureOpenAi,
}
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // qwen-* model names to the OpenAI-compat client pointed at DashScope.
    ...
    if canonical.starts_with("mistral") || canonical.starts_with("mixtral") {
        return Some(ProviderMetadata {
            provider: ProviderKind::Mistral,
            auth_env: "MISTRAL_API_KEY",
            base_url_env: "MISTRAL_BASE_URL",
    ...
    if canonical.starts_with("llama") || canonical.starts_with("groq") {
        return Some(ProviderMetadata {
            provider: ProviderKind::Groq,
            auth_env: "GROQ_API_KEY",
            base_url_env: "GROQ_BASE_URL",
    ...
    // T109: Bedrock — models prefixed with "bedrock/" or "anthropic.claude"
    if canonical.starts_with("bedrock/") || canonical.starts_with("anthropic.claude") {
        return Some(ProviderMetadata {
    ...
    // T110: Azure OpenAI — models prefixed with "azure/"
...
const FOREIGN_PROVIDER_ENV_VARS: &[(&str, &str, &str)] = &[
    ...
    (
        "DEEPSEEK_API_KEY",
        "DeepSeek",
        "prefix your model name with `deepseek` (e.g. `--model deepseek-chat`) so prefix routing selects the DeepSeek backend",
    ),
    (
        "DASHSCOPE_API_KEY",
        "Alibaba DashScope",
...
/// process environment or in the working-directory `.env` file.
...
mod tests {
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        let home = root.join("home").join(".agentii");
        std::fs::create_dir_all(cwd.join(".agentii")).expect("project config dir");
        ...
        // when
        let loaded = agentii_core::ConfigLoader::new(&cwd, &home)
            .load()
...
";
    ...
    #[test]
    fn anthropic_missing_credentials_hint_detects_openai_api_key() {
        let _lock = env_lock();
        let _openai = EnvVarGuard::set("OPENAI_API_KEY", Some("sk-test"));
        let _xai = EnvVarGuard::set("XAI_API_KEY", None);
        let _deepseek = EnvVarGuard::set("DEEPSEEK_API_KEY", None);
    ...
    #[test]
    fn anthropic_missing_credentials_builds_error_with_no_hint_when_clean() {
        let _lock = env_lock();
        let _openai = EnvVarGuard::set("OPENAI_API_KEY", None);
        let _xai = EnvVarGuard::set("XAI_API_KEY", None);
        let _deepseek = EnvVarGuard::set("DEEPSEEK_API_KEY", None);
    ...
    fn anthropic_missing_credentials_builds_error_with_hint_when_openai_key_is_set() {
        ...
        let error = anthropic_missing_credentials();
        match &error {
            ApiError::MissingCredentials { hint, .. } => {
    ...
    #[test]
    fn openai_prefix_routes_to_openai_provider() {
        let meta = super::metadata_for_model("openai/gpt-4.1-mini")
    ...
    #[test]
    fn gpt_prefix_routes_to_openai_provider() {
        let meta = super::metadata_for_model("gpt-4.1")
            .expect("gpt- prefix should resolve");
        assert_eq!(meta.provider, ProviderKind::OpenAi);
        assert_eq!(meta.auth_env, "OPENAI_API_KEY");
    }
    ...
    fn anthropic_missing_credentials_hint_detects_openai_api_key_and_recommends_openai_prefix() {
        ...
        assert!(
            ...
            "hint should name the detected env var: {hint}"
        );
        assert!(
        ...
        assert!(
            ...
            "hint should mention OPENAI_BASE_URL: {hint}"
        );
    }
    ...
    fn anthropic_missing_credentials_hint_prefers_openai_when_multiple_foreign_creds_set() {
        ...
        assert!(
            ...
            "OpenAI should be prioritized: {hint}"
        );
        assert!(
            !hint.contains("XAI_API_KEY"),
            "only the first detected provider should be named: {hint}"
        );
    }
