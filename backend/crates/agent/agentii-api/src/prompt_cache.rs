...
impl PromptCacheStats {
    /// T113: Compute prompt cache hit rate (0.0–1.0).
    ///
    /// Returns `None` when no requests have been tracked yet.
    #[must_use]
    pub fn hit_rate(&self) -> Option<f64> {
        ...
        if total == 0 {
            ...
            Some(self.completion_cache_hits as f64 / total as f64)
        }
    }

    /// Returns `true` when the hit rate meets the NFR-008 target of ≥80%.
    #[must_use]
    pub fn meets_nfr_008_target(&self) -> bool {
        self.hit_rate().map_or(false, |r| r >= 0.80)
    }
}
...
mod tests {
    ...
    #[test]
    fn meets_nfr_008_target_returns_true_when_hit_rate_at_least_80_percent() {
        // T113: NFR-008 — prompt cache hit rate target ≥80%
        use super::PromptCacheStats;

        // Exactly 80% hits → should pass
        let stats = PromptCacheStats {
            completion_cache_hits: 8,
            completion_cache_misses: 2,
            ..PromptCacheStats::default()
        };
        assert!(stats.meets_nfr_008_target(), "80% hit rate should meet NFR-008");

        // Above 80%
        let stats_high = PromptCacheStats {
            completion_cache_hits: 9,
            completion_cache_misses: 1,
            ..PromptCacheStats::default()
        };
        assert!(stats_high.meets_nfr_008_target(), "90% hit rate should meet NFR-008");

        // Below 80% → should fail
        let stats_low = PromptCacheStats {
            completion_cache_hits: 7,
            completion_cache_misses: 3,
            ..PromptCacheStats::default()
        };
        assert!(!stats_low.meets_nfr_008_target(), "70% hit rate should NOT meet NFR-008");

        // No requests yet → should return false (None hit rate)
        let stats_empty = PromptCacheStats::default();
        assert!(!stats_empty.meets_nfr_008_target(), "zero requests should NOT meet NFR-008");
    }
