...
use agentii_plugins::{PluginError, PluginLoadFailure, PluginManager, PluginSummary};
use agentii_core::{
...
pub fn handle_mcp_slash_command(
    args: Option<&str>,
    cwd: &Path,
) -> Result<String, agentii_core::ConfigError> {
...
pub fn handle_mcp_slash_command_json(
    args: Option<&str>,
    cwd: &Path,
) -> Result<Value, agentii_core::ConfigError> {
    let loader = ConfigLoader::default_for(cwd);
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            ...
            ancestor.join(".agentii").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(claw_config_home) = env::var("AGENTII_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".agentii").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".agentii").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaw,
            home.join(".agentii").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    ...
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".agentii").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set AGENTII_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Sources          .agentii/agents, ~/.agentii/agents, $AGENTII_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "sources": [".agentii/agents", "~/.agentii/agents", "$AGENTII_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Install root     $AGENTII_CONFIG_HOME/skills or ~/.agentii/skills".to_string(),
        "  Sources          .agentii/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.agentii/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "install_root": "$AGENTII_CONFIG_HOME/skills or ~/.agentii/skills",
            "sources": [
                ".agentii/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Sources          .agentii/settings.json, .agentii/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "sources": [".agentii/settings.json", ".agentii/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
mod tests {
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".agentii").join("skills");
        let legacy_commands = workspace.join(".agentii").join("commands");
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help
            .contains("Sources          .agentii/agents, ~/.agentii/agents, $AGENTII_CONFIG_HOME/agents"));
        ...
        assert!(skills_help.contains("Install root     $AGENTII_CONFIG_HOME/skills or ~/.agentii/skills"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".agentii")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".agentii").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".agentii").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".agentii/settings.json");
