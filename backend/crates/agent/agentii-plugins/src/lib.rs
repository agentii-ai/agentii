...
fn detect_claude_code_manifest_contract_gaps(
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field `skills` uses the Claude Code plugin contract; `claw` does not load plugin-managed skills and instead discovers skills from local roots such as `.agentii/skills`, `.omc/skills`, `.agents/skills`, `~/.omc/skills`, and `~/.claude/skills/omc-learned`.",
        ),
        (
            "mcpServers",
...
/// Guards against concurrent modification of `AGENTII_CONFIG_HOME`.
#[cfg(test)]
...
mod tests {
  ...
  "description": "Packaged manifest in install root"
    ...
    /// Regression test for ROADMAP #41: verify that `AGENTII_CONFIG_HOME` isolation prevents
    /// host `~/.agentii/plugins/` from bleeding into test runs.
    #[test]
    fn claw_config_home_isolation_prevents_host_plugin_leakage() {
        let _guard = env_guard();

        // Create a temp directory to act as our isolated AGENTII_CONFIG_HOME
        let config_home = temp_dir("isolated-home");
        let bundled_root = temp_dir("isolated-bundled");

        // Set AGENTII_CONFIG_HOME to our temp directory
  ...
  "description": "Test fixture plugin in isolated config home"
        ...
        // Create PluginManager with isolated bundled_root - it should use the temp config_home, not host ~/.agentii/
        let mut config = PluginManagerConfig::new(&config_home);
        config.bundled_root = Some(bundled_root.clone());
        let manager = PluginManager::new(config);
        ...
        assert_eq!(
            installed.len(),
            1,
            "should only see the test fixture plugin, not host ~/.agentii/plugins/"
        );
        assert_eq!(
