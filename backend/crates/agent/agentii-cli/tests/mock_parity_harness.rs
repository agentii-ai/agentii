...
use agentii_mock_service::{MockAnthropicService, SCENARIO_PREFIX};
...
fn clean_env_cli_reaches_mock_anthropic_service_across_scripted_parity_scenarios() {
    ...
    let cases = [
        ...
        ScenarioCase {
            ...
            allowed_tools: None,
            stdin: None,
            prepare: prepare_session_resume_fixture,
            assert: assert_session_resume,
            extra_env: None,
            resume_session: None,
        },
        ScenarioCase {
    ...
    assert_eq!(
        captured.len(),
        24,
        "fifteen scenarios should produce twenty-four requests"
    );
    assert!(captured
        .iter()
...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("AGENTII_CONFIG_HOME", &workspace.config_home)
...
fn prepare_auto_compact_fixture(workspace: &HarnessWorkspace) {
    let sessions_dir = workspace.root.join(".agentii").join("sessions");
...
fn assert_bash_permission_prompt_approved(_: &HarnessWorkspace, run: &ScenarioRun) {
    ...
    assert_eq!(
        run.response["tool_results"][0]["is_error"],
        Value::Bool(false),
        "tool_results[0].is_error should be false; full response: {}",
        serde_json::to_string_pretty(&run.response).unwrap_or_default()
    );
    let tool_output = run.response["tool_results"][0]["output"]
        .as_str()
        .expect("tool output");
...
fn prepare_session_resume_fixture(workspace: &HarnessWorkspace) {
    let sessions_dir = workspace.root.join(".agentii").join("sessions");
    fs::create_dir_all(&sessions_dir).expect("sessions dir should exist");
    let session_id = "parity-session-resume-seed";
    let session_jsonl = r#"{"type":"session_meta","version":3,"session_id":"parity-session-resume-seed","created_at_ms":1743724800000,"updated_at_ms":1743724800000}
{"type":"message","message":{"role":"user","blocks":[{"type":"text","text":"prior message from previous session"}]}}
{"type":"message","message":{"role":"assistant","blocks":[{"type":"text","text":"acknowledged prior message"}]}}
"#;
    fs::write(
        sessions_dir.join(format!("{session_id}.jsonl")),
        session_jsonl,
...
fn assert_session_resume(_: &HarnessWorkspace, run: &ScenarioRun) {
    assert_eq!(run.response["iterations"], Value::from(1));
    assert!(
        run.response["message"]
            .as_str()
            .expect("message text")
            .contains("session resume parity complete."),
        "expected session resume message"
    );
}

fn assert_agent_fork(_: &HarnessWorkspace, run: &ScenarioRun) {
    assert_eq!(run.response["iterations"], Value::from(1));
    let message = run.response["message"].as_str().expect("message text");
    assert!(
        message.contains("agent fork parity complete."),
        "expected agent fork message, got: {message}"
    );
    assert!(
        message.contains("fork-result-parity"),
        "expected sub-agent result marker in message, got: {message}"
    );
}

fn assert_error_recovery(_: &HarnessWorkspace, run: &ScenarioRun) {
    assert_eq!(run.response["iterations"], Value::from(1));
    assert!(
        run.response["message"]
            .as_str()
            .expect("message text")
            .contains("error recovery parity complete."),
        "expected error recovery message"
    );
}
