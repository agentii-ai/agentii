...
use agentii_core::ContentBlock;
use agentii_core::Session;
...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  agentii --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    let config_home = temp_dir.join("home").join(".agentii");
    fs::create_dir_all(project_dir.join(".agentii")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".agentii").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = run_claw_with_env(
        ...
        &[("AGENTII_CONFIG_HOME", config_home.to_str().expect("utf8 path"))],
    );
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    let project_dir = temp_dir.join("project");
    let sessions_dir = project_dir.join(".agentii").join("sessions");
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "agentii-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}

fn workspace_session(workspace: &Path) -> Session {
    let mut session = Session::new();
    session = session.with_workspace_root(workspace.to_path_buf());
    session
        .push_user_text("workspace session fixture")
        .expect("write ok");
    session
}
...
fn resumed_no_command_emits_restored_json() {
    let temp_dir = unique_temp_dir("resume-no-command-json");
    fs::create_dir_all(&temp_dir).expect("temp dir should exist");
    let session_path = temp_dir.join("session.jsonl");
    let mut session = workspace_session(&temp_dir);
...
fn resumed_stub_command_emits_not_implemented_json() {
    ...
    // /compact is interactive-only and should emit a not-implemented envelope
    let output = run_claw(
        &temp_dir,
        &[
            "--output-format",
    ...
    // Either the command succeeds with a JSON envelope or fails with a JSON error
    let parsed: Value = serde_json::from_str(stdout.trim())
        .or_else(|_| serde_json::from_str(stderr.trim()))
        .expect("should produce json output");
    // The envelope should indicate the command is not supported in non-interactive mode
    assert!(
        parsed.get("kind").is_some() || parsed.get("error").is_some(),
        "stub command should produce structured output, got: {parsed}"
    );
}
