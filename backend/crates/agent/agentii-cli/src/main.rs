...
use agentii_compat_harness::{extract_manifest, UpstreamPaths};
use init::initialize_repo;
use agentii_plugins::{PluginHooks, PluginManager, PluginManagerConfig, PluginRegistry};
...
const OFFICIAL_REPO_URL: &str = "https://github.com/agentii-ai/agentii";
const OFFICIAL_REPO_SLUG: &str = "agentii-ai/agentii";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install agentii";
...
fn main() {
    if let Err(error) = run() {
        ...
        } else if message.contains("`agentii --help`") {
            eprintln!("error: {message}");
        } else {
            eprintln!(
                "error: {message}

Run `agentii --help` for usage."
            );
        }
        std::process::exit(1);
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    ) =>
            {
                ...
                // the arg to the API (e.g. `agentii prompt --help`) should show
            ...
            flag if flag.starts_with("--output-format=") || flag.starts_with("--format=") => {
                let eq_pos = flag.find('=').unwrap();
                output_format = CliOutputFormat::parse(&flag[eq_pos + 1..])?;
                index += 1;
            }
            ...
            "--tier" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| "missing value for --tier".to_string())?;
                permission_mode_override = Some(parse_tier_arg(value)?);
                index += 2;
            }
            flag if flag.starts_with("--tier=") => {
                permission_mode_override = Some(parse_tier_arg(&flag[7..])?);
                index += 1;
            }
            "--compact" => {
                compact = true;
            ...
            "-p" => {
                // Agentii compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Agentii compat: --print makes output non-interactive
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`agentii {command_name}` is a slash command. Use `agentii --resume SESSION.jsonl /{command_name}` or start `agentii` and run `/{command_name}`."
        )
    } else {
        format!(
            "`agentii {command_name}` is a slash command. Start `agentii` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn removed_auth_surface_error(command_name: &str) -> String {
    format!(
        "`agentii {command_name}` has been removed. Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
    )
}
...
fn parse_acp_args(args: &[String], output_format: CliOutputFormat) -> Result<CliAction, String> {
    match args {
        ...
        _ => Err(String::from(
            "unsupported ACP invocation. Use `agentii acp`, `agentii acp serve`, `agentii --acp`, or `agentii -acp`.",
        )),
    }
}
...
fn parse_direct_slash_cli_action(
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `agentii` and run it there, or use `agentii --resume SESSION.jsonl {command_name}` / `agentii --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `agentii --help` for CLI usage, or start `agentii` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `agentii` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
...
fn parse_tier_arg(value: &str) -> Result<PermissionMode, String> {
    match value {
        "suggest" => Ok(PermissionMode::ReadOnly),
        "auto-edit" => Ok(PermissionMode::WorkspaceWrite),
        "full-auto" => Ok(PermissionMode::DangerFullAccess),
        other => Err(format!(
            "unsupported tier '{other}'. Use suggest, auto-edit, or full-auto."
        )),
    }
}
...
fn render_doctor_report() -> Result<DoctorReport, Box<dyn std::error::Error>> {
    ...
    let empty_config = agentii_core::RuntimeConfig::empty();
...
/// Starts a minimal Model Context Protocol server that exposes agentii's
/// built-in tools over stdio.
///
/// Tool descriptors come from [`agentii_tools::mvp_tool_specs`] and calls are
/// dispatched through [`agentii_tools::execute_tool`], so this server exposes exactly
/// Read `.agentii/worker-state.json` from the current working directory and print it.
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let state_path = cwd.join(".agentii").join("worker-state.json");
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `agentii login` is removed"
                .to_string(),
        ])
...
fn check_config_health(
    ...
    config: Result<&agentii_core::RuntimeConfig, &agentii_core::ConfigError>,
) -> DiagnosticCheck {
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `agentii` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
fn check_sandbox_health(status: &agentii_core::SandboxStatus) -> DiagnosticCheck {
...
fn check_system_health(cwd: &Path, config: Option<&agentii_core::RuntimeConfig>) -> DiagnosticCheck {
    let default_model = config.and_then(agentii_core::RuntimeConfig::model);
    let mut details = vec![
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set CLAUDE_CODE_UPSTREAM=/path/to/upstream or pass `agentii dump-manifests --manifests-dir /path/to/upstream`.";
...
fn print_bootstrap_plan(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let phases = agentii_core::BootstrapPlan::claude_code_default()
        .phases()
        .iter()
...
struct StatusContext {
    ...
    sandbox_status: agentii_core::SandboxStatus,
}
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .agentii/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Compact => {
            let result = agentii_core::compact_session(
                session,
                CompactionConfig {
        ...
        SlashCommand::Clear { confirm } => {
            ...
            Ok(ResumeCommandOutcome {
                session: cleared,
                message: Some(format!(
                    "Session cleared\n  Mode             resumed session reset\n  Previous session {previous_session_id}\n  Backup           {}\n  Resume previous  agentii --resume {}\n  New session      {new_session_id}\n  Session file     {}",
                    backup_path.display(),
                    backup_path.display(),
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                return Err(
                    "resumed /skills invocations are interactive-only; start `agentii` and run `/skills <skill>` in the REPL".into(),
                );
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: agentii is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && agentii",
            cwd.display()
        );
        ...
        let message = format!(
            "agentii is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && agentii",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
...
struct RuntimePluginState {
    feature_config: agentii_core::RuntimeFeatureConfig,
    tool_registry: GlobalToolRegistry,
...
struct RuntimeMcpState {
    ...
    degraded_report: Option<agentii_core::McpDegradedReport>,
}
...
impl BuiltRuntime {
    ...
    fn with_hook_abort_signal(mut self, hook_abort_signal: agentii_core::HookAbortSignal) -> Self {
        let runtime = self
            .runtime
            .take()
...
impl RuntimeMcpState {
    fn new(
        runtime_config: &agentii_core::RuntimeConfig,
    ) -> Result<Option<(Self, agentii_core::McpToolDiscoveryReport)>, Box<dyn std::error::Error>> {
        ...
        let failed_servers =
            discovery
                .failed_servers
                .iter()
                .map(|failure| agentii_core::McpFailedServer {
                    server_name: failure.server_name.clone(),
                    phase: agentii_core::McpLifecyclePhase::ToolDiscovery,
                    error: agentii_core::McpErrorSurface::new(
                        agentii_core::McpLifecyclePhase::ToolDiscovery,
                ...
                .chain(discovery.unsupported_servers.iter().map(|server| {
                    agentii_core::McpFailedServer {
                        server_name: server.server_name.clone(),
                        phase: agentii_core::McpLifecyclePhase::ServerRegistration,
                        error: agentii_core::McpErrorSurface::new(
                            agentii_core::McpLifecyclePhase::ServerRegistration,
        ...
        let degraded_report = (!failed_servers.is_empty()).then(|| {
            agentii_core::McpDegradedReport::new(
                working_servers,
                failed_servers,
    ...
    fn degraded_report(&self) -> Option<agentii_core::McpDegradedReport> {
...
fn mcp_runtime_tool_definition(tool: &agentii_core::ManagedMcpTool) -> RuntimeToolDefinition {
...
impl HookAbortMonitor {
    fn spawn(abort_signal: agentii_core::HookAbortSignal) -> Self {
    ...
    fn spawn_with_waiter<F>(abort_signal: agentii_core::HookAbortSignal, wait_for_interrupt: F) -> Self
    where
        F: FnOnce(Receiver<()>, agentii_core::HookAbortSignal) + Send + 'static,
    {
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn prepare_turn_runtime(
        ...
        let hook_abort_signal = agentii_core::HookAbortSignal::new();
        let runtime = build_runtime(
                ...
                "estimated_cost": format_usd(
                    summary.usage.estimate_cost_usd_with_pricing(
                        pricing_for_model(&self.model)
                            .unwrap_or_else(agentii_core::ModelPricing::default_sonnet_tier)
                    ).total_cost_usd()
                )
            })
        );
    ...
    fn print_sandbox_status() {
        ...
        let runtime_config = loader
            .load()
            .unwrap_or_else(|_| agentii_core::RuntimeConfig::empty());
        println!(
            "{}",
    ...
    fn print_mcp(
        ...
        // `agentii mcp serve` starts a stdio MCP server exposing agentii's built-in
...
fn current_session_store() -> Result<agentii_core::SessionStore, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    agentii_core::SessionStore::from_cwd(&cwd).map_err(|e| Box::new(e) as Box<dyn std::error::Error>)
}
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .agentii/sessions/<session-id>.jsonl".to_string(),
...
fn status_json_value(
    ...
    json!({
        "kind": "status",
        "model": model.unwrap_or("restored-session"),
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .agentii/sessions/. Extract the stem (drop the .jsonl extension).
...
fn format_sandbox_report(status: &agentii_core::SandboxStatus) -> String {
    format!(
        "Sandbox
  Enabled           {}
  Active            {}
...
fn sandbox_json_value(status: &agentii_core::SandboxStatus) -> serde_json::Value {
    json!({
        "kind": "sandbox",
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            agentii status
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Related          /status · agentii --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            agentii sandbox
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Related          /sandbox · agentii status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            agentii doctor
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Related          /doctor · agentii --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            agentii acp [serve]
  Aliases          agentii --acp · agentii -acp
  ...
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · agentii --help"
            .to_string(),
    }
}
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let message = "ACP/Zed editor integration is not implemented in agentii yet. `agentii acp serve` is only a discoverability alias today; it does not launch a daemon or Zed-specific protocol endpoint. Use the normal terminal surfaces for now and track ROADMAP #76 for real ACP support.";
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           discoverability only\n  Launch           `agentii acp serve` / `agentii --acp` / `agentii -acp` report status only; no editor daemon is available yet\n  Today            use `agentii prompt`, the REPL, or `agentii doctor` for local verification\n  Tracking         ROADMAP #76\n  Message          {message}"
            );
        }
        CliOutputFormat::Json => {
            println!(
...
fn runtime_hook_config_from_plugin_hooks(hooks: PluginHooks) -> agentii_core::RuntimeHookConfig {
    agentii_core::RuntimeHookConfig::new(
        hooks.pre_tool_use,
        hooks.post_tool_use,
...
impl agentii_core::HookProgressReporter for CliHookProgressReporter {
    fn on_event(&mut self, event: &agentii_core::HookProgressEvent) {
        match event {
            agentii_core::HookProgressEvent::Started {
                event,
                tool_name,
                command,
            } => eprintln!(
            ...
            agentii_core::HookProgressEvent::Completed {
                event,
                tool_name,
                command,
            } => eprintln!(
            ...
            agentii_core::HookProgressEvent::Cancelled {
                event,
                tool_name,
                command,
            } => eprintln!(
...
impl agentii_core::PermissionPrompter for CliPermissionPrompter {
    fn decide(
        &mut self,
        request: &agentii_core::PermissionRequest,
    ) -> agentii_core::PermissionPromptDecision {
        println!();
        ...
        match io::stdin().read_line(&mut response) {
            Ok(_) => {
                ...
                if matches!(normalized.as_str(), "y" | "yes") {
                    agentii_core::PermissionPromptDecision::Allow
                } else {
                    agentii_core::PermissionPromptDecision::Deny {
                        reason: format!(
            ...
            Err(error) => agentii_core::PermissionPromptDecision::Deny {
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // (`crates/agent/agentii-cli/tests/compact_output.rs`) to point
        // agentii at its fake Anthropic endpoint. We also attach a
        // session-scoped prompt cache on the Anthropic path; the
        // prompt cache is Anthropic-only so non-Anthropic variants
        // skip it.
        let resolved_model = agentii_api::resolve_model_alias(&model);
        let client = match detect_provider_kind(&resolved_model) {
            ProviderKind::Anthropic => {
                let auth = resolve_cli_auth_source()?;
                let inner = AnthropicClient::from_auth(auth)
                    .with_base_url(agentii_api::read_base_url())
...
fn resolve_cli_auth_source_for_cwd() -> Result<AuthSource, agentii_api::ApiError> {
...
fn format_user_visible_api_error(session_id: &str, error: &agentii_api::ApiError) -> String {
...
fn format_context_window_blocked_error(session_id: &str, error: &agentii_api::ApiError) -> String {
    let mut lines = vec![
    ...
    match error {
        agentii_api::ApiError::ContextWindowExceeded {
            model,
        ...
        agentii_api::ApiError::Api { message, body, .. } => {
        ...
        agentii_api::ApiError::RetriesExhausted { last_error, .. } => {
            let detail = match last_error.as_ref() {
                agentii_api::ApiError::Api { message, body, .. } => message.as_deref().unwrap_or(body),
    ...
    lines.push(format!(
        "  Resume compact   agentii --resume {session_id} /compact"
    ));
...
fn final_assistant_text(summary: &agentii_core::TurnSummary) -> String {
    summary
        .assistant_messages
        .last()
        .map(|message| {
...
fn collect_tool_uses(summary: &agentii_core::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .assistant_messages
        .iter()
...
fn collect_tool_results(summary: &agentii_core::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .tool_results
        .iter()
        .flat_map(|message| message.blocks.iter())
...
fn collect_prompt_cache_events(summary: &agentii_core::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .prompt_cache_events
        .iter()
        .map(|event| {
...
fn prompt_cache_record_to_runtime_event(
    record: agentii_api::PromptCacheRecord,
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "agentii v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  agentii [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  agentii [--model MODEL] [--output-format text|json] prompt TEXT"
    )?;
    writeln!(out, "      Send one prompt and exit")?;
    writeln!(
        out,
        "  agentii [--model MODEL] [--output-format text|json] TEXT"
    )?;
    writeln!(out, "      Shorthand non-interactive prompt mode")?;
    writeln!(
        out,
        "  agentii --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  agentii help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  agentii version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  agentii status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  agentii sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  agentii doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  agentii acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  agentii dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  agentii bootstrap-plan")?;
    writeln!(out, "  agentii agents")?;
    writeln!(out, "  agentii mcp")?;
    writeln!(out, "  agentii skills")?;
    writeln!(out, "  agentii system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(out, "  agentii init")?;
    writeln!(
        out,
        "  agentii export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .agentii/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  agentii --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  agentii --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  agentii --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  agentii --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  agentii --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  agentii --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  agentii agents")?;
    writeln!(out, "  agentii mcp show my-server")?;
    writeln!(out, "  agentii /skills")?;
    writeln!(out, "  agentii doctor")?;
    ...
    writeln!(out, "  agentii init")?;
    writeln!(out, "  agentii export")?;
    writeln!(out, "  agentii export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    use agentii_api::{ApiError, MessageResponse, OutputContentBlock, Usage};
    use agentii_plugins::{
    ...
    use agentii_tools::GlobalToolRegistry;
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   agentii --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("agentii-cli-{nanos}-{unique}"))
    }
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".agentii")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".agentii").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("AGENTII_CONFIG_HOME").ok();
        let original_permission_mode = std::env::var("RUSTY_CLAUDE_PERMISSION_MODE").ok();
        std::env::set_var("AGENTII_CONFIG_HOME", &config_home);
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");

        let resolved = with_current_dir(&cwd, super::default_permission_mode);

        match original_config_home {
            Some(value) => std::env::set_var("AGENTII_CONFIG_HOME", value),
            None => std::env::remove_var("AGENTII_CONFIG_HOME"),
        }
    ...
    fn resolve_cli_auth_source_ignores_saved_oauth_credentials() {
        ...
        save_oauth_credentials(&agentii_core::OAuthTokenSet {
    ...
    fn parses_direct_agents_mcp_and_skills_slash_commands() {
        ...
        assert!(error.contains("agentii --resume SESSION.jsonl /status"));
    }
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("agentii --help"));
    }
    ...
    fn permission_policy_uses_plugin_tool_permissions() {
        let feature_config = agentii_core::RuntimeFeatureConfig::default();
    ...
    fn shared_help_uses_resume_annotation_copy() {
        let help = agentii_commands::render_slash_command_help();
    ...
    fn repl_help_includes_shared_commands_and_exit() {
        ...
        assert!(help.contains("Auto-save            .agentii/sessions/<session-id>.jsonl"));
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        std::env::remove_var("ANTHROPIC_MODEL");
        std::env::remove_var("AGENTII_CONFIG_HOME");
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }
    ...
    fn cost_report_uses_sectioned_layout() {
        let report = format_cost_report(agentii_core::TokenUsage {
            input_tokens: 20,
            output_tokens: 8,
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("agentii help"));
        assert!(help.contains("agentii version"));
        assert!(help.contains("agentii status"));
        assert!(help.contains("agentii sandbox"));
        assert!(help.contains("agentii init"));
        assert!(help.contains("agentii acp [serve]"));
        assert!(help.contains("agentii agents"));
        assert!(help.contains("agentii mcp"));
        assert!(help.contains("agentii skills"));
        assert!(help.contains("agentii /skills"));
        assert!(help.contains("agentii-ai/agentii"));
        assert!(help.contains("cargo install agentii"));
        assert!(!help.contains("agentii login"));
        assert!(!help.contains("agentii logout"));
    }
    ...
    fn status_line_reports_model_and_token_totals() {
        let status = format_status_report(
            ...
            StatusUsage {
                message_count: 7,
                turns: 3,
                latest: agentii_core::TokenUsage {
                    input_tokens: 5,
                    output_tokens: 4,
                    cache_creation_input_tokens: 1,
                    cache_read_input_tokens: 0,
                },
                cumulative: agentii_core::TokenUsage {
                    input_tokens: 20,
                    output_tokens: 8,
            ...
            &super::StatusContext {
                ...
                sandbox_status: agentii_core::SandboxStatus::default(),
            },
        );
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("agentii --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("agentii --resume latest"));
        assert!(help.contains("agentii --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".agentii/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".agentii/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".agentii/sessions/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("agentii-cli-{label}-{nanos}"))
    }
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading crates/agent/agentii-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p agentii-cli"}"#)
                .contains("cargo test -p agentii-cli")
        );
        assert_eq!(
...
mod sandbox_report_tests {
    ...
    use agentii_core::HookAbortSignal;
    use std::sync::mpsc;
    use std::time::Duration;

    #[test]
    fn sandbox_report_renders_expected_fields() {
        let report = format_sandbox_report(&agentii_core::SandboxStatus::default());
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_shows_helpful_error_when_manifests_missing() {
        let root = std::env::temp_dir().join(format!(
            "agentii_test_missing_manifests_{}",
            std::process::id()
        ));
    ...
    fn dump_manifests_uses_explicit_manifest_dir() {
        let root = std::env::temp_dir().join(format!(
            "agentii_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
