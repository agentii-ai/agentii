...
const STARTER_AGENTII_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Agentii local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".agentii/settings.local.json", ".agentii/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_dir = cwd.join(".agentii");
    artifacts.push(InitArtifact {
        name: ".agentii/",
        status: ensure_dir(&claw_dir)?,
    });

    let claw_json = cwd.join(".agentii.json");
    artifacts.push(InitArtifact {
        name: ".agentii.json",
        status: write_file_if_missing(&claw_json, STARTER_AGENTII_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.agentii.json`; reserve `.agentii/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("rusty-claude-init-{}-{}-{nanos}", std::process::id(), counter))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".agentii/"));
        assert!(rendered.contains(".agentii.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".agentii").is_dir());
        assert!(root.join(".agentii.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".agentii.json")).expect("read claw json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".agentii/settings.local.json"));
        assert!(gitignore.contains(".agentii/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".agentii/settings.local.json\n").expect("write gitignore");
        ...
        assert!(second_rendered.contains(".agentii/"));
        assert!(second_rendered.contains(".agentii.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".agentii/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".agentii/sessions/").count(), 1);
