...
use agentii_financial_tools::execute_financial_tool;
...
use agentii_plugins::PluginTool;
...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            ...
            description: "Search for financial documents (SEC filings, PDFs, clinical trials) by ticker, year, source type, or date range. Returns a paginated list of matching documents with metadata. Use this as the FIRST step in financial research to identify relevant documents. When NOT to use: for general web search or non-financial documents.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "ticker": { "type": "string", "description": "Stock ticker symbol (e.g., NVDA, AAPL)" },
                    "year": { "type": "integer", "description": "Fiscal year to filter by" },
                    "source_type": { "type": "string", "enum": ["clinical_trials", "faers", "fda_approvals", "sec_filing", "pdf"], "description": "Type of document source" },
        ...
        ToolSpec {
            ...
            description: "Get the table of contents (outline) for a specific document. Use AFTER fetch_filtered_document_names to identify which sections contain the data you need.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
        ...
        ToolSpec {
            ...
            description: "Fetch specific pages from a financial document by row numbers. Use AFTER fetch_document_outline to get targeted content. Format citations as [ref](ref_id-row_number).",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "source_id": { "type": "string", "description": "Document source ID" },
                    "row_numbers": { "type": "array", "items": { "type": "integer" }, "description": "Row numbers to fetch from the document" }
                },
        ...
        ToolSpec {
            ...
            description: "Full-text keyword search within a specific financial document. Use when you need to find specific terms or phrases. Do NOT use as the first step — use fetch_document_outline first.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "source_id": { "type": "string", "description": "Document source ID" },
        ...
        ToolSpec {
            ...
            description: "Fetch structured financial statement data (income statement, balance sheet, cash flow) for a company. Returns standardized financial data with line items and values.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    ...
                    "fiscal_periods": { "type": "array", "items": { "type": "string" }, "description": "Fiscal periods to fetch (e.g., ['FY2024', 'FY2023'])" }
                },
        ...
        ToolSpec {
            ...
            description: "Get company profile and stock information for a ticker. Returns sector, industry, market cap, and other company metadata.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "ticker": { "type": "string", "description": "Stock ticker symbol" }
        ...
        ToolSpec {
            ...
            description: "Read the current market context snapshot from the workspace. Returns today's market data, portfolio positions, and watchlist from .agentii/market-context/snapshot.json.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {},
                "additionalProperties": false
        ...
        ToolSpec {
            name: "stage_order",
            description: "Stage a trading order for review. Writes to .agentii/orders/staged.json for IDE confirmation. Does NOT execute the order.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        name if matches!(name, "fetch_filtered_document_names" | "fetch_document_outline" | "fetch_document_chunk_content" | "search_keyword_in_source" | "fetch_financial_statement" | "fetch_stock_info" | "read_market_context" | "stage_order") => {
            let workspace_root = std::env::current_dir().unwrap_or_default();
            execute_financial_tool(name, input, workspace_root.to_str().unwrap_or("."))
...
fn resolve_skill_path(skill: &str) -> Result<std::path::PathBuf, String> {
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    match agentii_commands::resolve_skill_path(&cwd, skill) {
        Ok(path) => Ok(path),
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(claw_config_home) = std::env::var("AGENTII_CONFIG_HOME") {
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.agentii/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".agentii"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".agentii"));
...
fn build_agent_system_prompt(subagent_type: &str) -> Result<Vec<String>, String> {
    ...
    if subagent_type == "financial-retrieval" {
        prompt.push(agentii_prompts::financial_retrieval_agent_section("", ""));
    }
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "clawguide" | "clawguideagent" | "guide" | "agentiiguide" => String::from("agentii-guide"),
        "statusline" | "statuslinesetup" => String::from("statusline-setup"),
        "financialretrieval" | "financialretrievalagent" | "retrieval" => String::from("financial-retrieval"),
        _ => trimmed.to_string(),
    }
}
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".agentii").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("AGENTII_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use AGENTII_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".agentii"))
}
...
#[allow(clippy::needless_pass_by_value)]
fn execute_powershell(input: PowerShellInput) -> std::io::Result<agentii_core::BashCommandOutput> {
...
fn execute_shell_command(
    ...
    timeout: Option<u64>,
    run_in_background: Option<bool>,
) -> std::io::Result<agentii_core::BashCommandOutput> {
    if run_in_background.unwrap_or(false) {
        let child = std::process::Command::new(shell)
            ...
            .spawn()?;
        return Ok(agentii_core::BashCommandOutput {
            stdout: String::new(),
            stderr: String::new(),
            raw_output_path: None,
            interrupted: false,
...
Command exceeded timeout of {timeout_ms} ms",
    ...
    Ok(agentii_core::BashCommandOutput {
        stdout: String::from_utf8_lossy(&output.stdout).into_owned(),
        stderr: String::from_utf8_lossy(&output.stderr).into_owned(),
        raw_output_path: None,
        interrupted: false,
...
mod tests {
    ...
    use agentii_api::OutputContentBlock;
    use agentii_core::ProviderFallbackConfig;
    use agentii_core::{
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .agentii/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let claw_dir = worktree.join(".agentii");
        fs::create_dir_all(&claw_dir).expect("create .agentii dir");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .agentii/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".agentii").join("worker-state.json");
    ...
    fn runtime_tools_extend_registry_definitions_permissions_and_search() {
        let registry = GlobalToolRegistry::builtin()
            .with_runtime_tools(vec![super::RuntimeToolDefinition {
                ...
                required_permission: agentii_core::PermissionMode::ReadOnly,
            }])
        ...
        assert_eq!(
            ...
            vec![(
                ...
                agentii_core::PermissionMode::ReadOnly
            )]
        );
        ...
        let search = registry.search(
            ...
            Some(agentii_core::McpDegradedReport::new(
                vec!["demo".to_string()],
                vec![agentii_core::McpFailedServer {
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".agentii").join("skills").join("plan");
        let command_dir = root.join(".agentii").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".agentii/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".agentii/commands/handoff.md"));
    ...
    impl agentii_core::ApiClient for MockSubagentApiClient {
        fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
            self.calls += 1;
            match self.calls {
                1 => {
    ...
    fn subagent_runtime_executes_tool_loop_with_isolated_session() {
        ...
        assert!(runtime
            ...
            .any(|block| matches!(
                block,
                agentii_core::ContentBlock::ToolResult { output, .. }
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".agentii")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".agentii")).expect("cwd dir");
        std::fs::write(
            home.join(".agentii").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".agentii")).expect("cwd dir");
        std::fs::write(
            cwd.join(".agentii").join("settings.local.json"),
        ...
        let local_settings = std::fs::read_to_string(cwd.join(".agentii").join("settings.local.json"))
            .expect("local settings after enter");
        assert!(local_settings.contains(r#""defaultMode": "plan""#));
        let state =
            std::fs::read_to_string(cwd.join(".agentii").join("tool-state").join("plan-mode.json"))
...
"#,
    ...
    fn read_only_registry() -> super::GlobalToolRegistry {
        use agentii_core::permission_enforcer::PermissionEnforcer;
        use agentii_core::PermissionPolicy;

        let policy = mvp_tool_specs().into_iter().fold(
            PermissionPolicy::new(agentii_core::PermissionMode::ReadOnly),
            |policy, spec| policy.with_tool_requirement(spec.name, spec.required_permission),
        );
    ...
    fn run_task_packet_creates_packet_backed_task() {
        use agentii_core::task_packet::TaskScope;
